...
export const createEditorShellController = ({
  ...
  const outlineToggleLabel = $derived(outlineOpen ? 'Close directory' : 'Open directory');
  const outlineControlDisabled = $derived(!outlineOpen && !sidePanelsAvailable);
  const syntaxToggleLabel = $derived(syntaxOpen ? 'Close syntax example' : 'Open grammar example');
  const syntaxControlDisabled = $derived(!syntaxOpen && !sidePanelsAvailable);
  const lineNumbersToggleLabel = $derived(lineNumbers ? 'Hide line numbers' : 'Show line number');
