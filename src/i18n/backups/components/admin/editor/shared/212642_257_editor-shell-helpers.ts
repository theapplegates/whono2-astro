...
export const EDITOR_SINGLE_VIEW_RETURN_LABEL = 'Return to editing and preview dual-area view';
...
export const getEditorLayoutToggleLabel = ({
  ...
  splitBothIsCompact
    ? 'Expand upper and lower dual areas'
    : stackedCanReturnToCompact
      ? 'Return to single area view'
      : editorLayout === 'split'
        ? 'Switch to top-bottom layout'
        : 'Switch to left and right layout';
...
export const getEditorPreviewViewToggleLabel = (editorViewMode: EditorViewMode): string =>
  editorViewMode === 'preview' ? 'Cancel preview view only' : 'Show preview area only';
...
export const getEditorCompactPaneToggleLabel = (compactPaneMode: EditorPaneMode): string =>
  compactPaneMode === 'edit' ? 'Show preview area' : 'Show editing area';
...
export const getEditorScrollSyncToggleLabel = ({
  ...
  available ? (enabled ? 'Turn off synchronous scrolling' : 'Turn on synchronous scrolling') : 'Synchronous scrolling is not possible in single view';
