...
const resolveFieldImageTarget = (field: AdminImageFieldContext, rawValue: string): FieldImageTarget => {
  ...
  if (field === 'bits.images') {
    ...
    if (!normalized) {
      throw new AdminImageError('bits.images only allowed public/** Relative image path under or https:// remote URL');
    }
  ...
  if (!localPath) {
    throw new AdminImageError('Hero The image address does not support the current local path format.');
  }
