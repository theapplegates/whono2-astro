...
export const initAboutSiteInfoCopyButtons = () => {
  ...
  document.addEventListener('click', async (event) => {
    ...
    button.dataset.state = 'copied';
    button.setAttribute('aria-label', 'Friendly link information has been copied');
    button.setAttribute('title', 'Friendly link information has been copied');
    window.setTimeout(() => {
      button.dataset.state = 'idle';
      button.setAttribute('aria-label', 'Copy friend link information');
      button.setAttribute('title', 'Copy friend link information');
    }, 2000);
  });
};
