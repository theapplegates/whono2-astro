...
export const createSocialLinks = ({
  ...
  const syncPresetRow = (row: Element | null): void => {
    ...
    toggleBtn.setAttribute('aria-label', visible ? `hide ${label}` : `recover ${label}`);
    toggleBtn.setAttribute('title', visible ? `hide ${label}` : `recover ${label}`);
  };
  ...
  const syncCustomVisibilityButton = (row: HTMLElement): void => {
    ...
    toggleBtn.dataset.state = visible ? 'visible' : 'hidden';
    toggleBtn.setAttribute('aria-pressed', visible ? 'true' : 'false');
    toggleBtn.setAttribute('aria-label', visible ? 'Hidden link' : 'show link');
    toggleBtn.setAttribute('title', visible ? 'Hidden link' : 'show link');
    toggleLabel.textContent = visible ? 'Hidden link' : 'show link';
  };
  ...
  const updateCustomRowsUi = (): void => {
    ...
    socialCustomCountEl.textContent = `(New ${rows.length} / ${ADMIN_SOCIAL_CUSTOM_LIMIT})`;
