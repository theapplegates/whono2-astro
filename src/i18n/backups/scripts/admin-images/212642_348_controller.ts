...
export const initAdminImagesConsole = () => {
  ...
  if (!bootstrap) {
    ...
    statusEl.textContent = 'Gallery initialization failed';
    statusLiveEl.textContent = 'Gallery initialization failed';
    return;
  }
  ...
  const syncControls = () => {
    ...
    filterToggleBtn.setAttribute('aria-label', filterVisible ? 'Close image filter' : 'Expand image filter');
    ...
    recentBtn.textContent = currentState.scope === 'recent' ? 'Return to category' : ADMIN_IMAGE_SCOPE_LABELS.recent;
  ...
  const loadDetailMeta = async (
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to read picture information';
  ...
  const applyResolvedLocalState = async ({
    ...
    if (shouldPrefetch) {
      busy = true;
      syncControls();
      setStatus('loading', 'Reading image information of current page...', false);
  ...
  const applyBrowseState = async ({ updateLocation }: { updateLocation: boolean }) => {
    ...
    await applyResolvedLocalState({
      ...
      updateLocation,
      successMessage: browsePage.totalCount > 0 ? `Matched ${browsePage.totalCount} pictures` : 'No matching pictures found'
    });
  };
  ...
  const loadList = async ({ updateLocation }: { updateLocation: boolean }) => {
    ...
    try {
      ...
      setStatus('ok', result.totalCount > 0 ? `Loaded ${result.totalCount} pictures` : 'No matching pictures found', false);
    } catch (error) {
      if (token !== requestToken) return;
      clearLoadFailure(error instanceof Error ? error.message : 'Image list reading failed');
    } finally {
  ...
  detailEl.addEventListener('click', async (event) => {
    ...
    try {
      ...
      setStatus('error', error instanceof Error ? error.message : `copy${copyLabel}fail`);
    }
  });
