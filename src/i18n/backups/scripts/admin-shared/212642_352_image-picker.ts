...
export const createAdminImagePicker = (root: ParentNode = document): AdminImagePickerController | null => {
  ...
  const getCurrentMarker = () => {
    ...
    if (fallbackCurrentValue.length > 0) {
      return {
        ...
        label: fallbackCurrentLabel || 'currently in use'
      };
    }
  ...
  const renderItems = (items: readonly AdminImageClientItem[], totalCount: number) => {
    ...
    items.forEach((item) => {
      ...
      if (isCurrent) {
        ...
        currentBadge.textContent = currentMarker?.label ?? 'currently in use';
  ...
  const loadList = async () => {
    ...
    try {
      ...
      if (token !== requestToken) return;
      console.warn('[admin-images-picker] Image list loading failed', error);
      currentItems = [];
      currentTotalCount = 0;
      syncSelectedItemFromCurrentItems();
      totalPages = 1;
      syncPager();
      resultsEl.replaceChildren();
      setStatus(error instanceof Error ? error.message : 'Loading failed');
    } finally {
  ...
  const open = (options: AdminImagePickerOpenOptions) => {
    ...
    resetBtn.textContent = options.resetLabel?.trim() || 'Restore default';
